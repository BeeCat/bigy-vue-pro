<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="班级学生" prop="studentCode">
        <el-input v-model="queryParams.studentCode" placeholder="请输入班级学生" clearable size="small" @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="班级姓名" prop="studentName">
        <el-input v-model="queryParams.studentName" placeholder="请输入班级姓名" clearable size="small" @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['zhh:class-student:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
                   v-hasPermi="['zhh:class-student:export']">导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="班级学生" prop="studentCode">
          <el-input v-model="form.studentCode" placeholder="请输入班级学生" />
        </el-form-item>
        <el-form-item label="班级姓名" prop="studentName">
          <el-input v-model="form.studentName" placeholder="请输入班级姓名" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // 执行查询
      getClassStudentPage(params).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateClassStudent(this.form).then(response => {
        ...
        // 添加的提交
        createClassStudent(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$confirm('是否确认删除班级学生编号为"' + id + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.addBeginAndEndTime(params, this.dateRangeCreateTime, 'createTime');
      // 执行导出
      this.$confirm('是否确认导出所有班级学生数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportClassStudentExcel(params);
        }).then(response => {
          this.downloadExcel(response, '班级学生.xls');
        })
    }
  }
};
