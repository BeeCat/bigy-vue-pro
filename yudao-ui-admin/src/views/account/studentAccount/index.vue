<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['zhh:student-account:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
                   v-hasPermi="['zhh:student-account:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="学员编码" align="center" prop="studentId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['zhh:student-account:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['zhh:student-account:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="姓名" prop="name">
           <el-select v-model="form.studentId" filterable @change="nameChange()"  placeholder="请选择">
              <el-option v-for="item in studentList" :key="item.id" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="课程编码" prop="classCode">
         <el-select v-model="form.classCode" placeholder="请选课程" clearable style="width: 100%">
        ...
        <el-form-item label="已消课程数" prop="costClassAmount">
          <el-input v-model="form.costClassAmount" placeholder="请输入已消课程数" />
        </el-form-item>
...
<script>
import { createStudentAccount, updateStudentAccount, deleteStudentAccount, getStudentAccount, getStudentAccountPage, exportStudentAccountExcel } from "@/api/account/studentAccount";
...
export default {
  ...
  created() {
    this.getList();
    getPage({pageSize:10000}).then(response => {
  ...
  methods: {
    ...
    getList() {
      ...
      // 执行查询
      getStudentAccountPage(params).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getStudentAccount(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateStudentAccount(this.form).then(response => {
        ...
        // 添加的提交
        createStudentAccount(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有学生账户数据项?', "警告", {
          ...
          return exportStudentAccountExcel(params);
        }).then(response => {
